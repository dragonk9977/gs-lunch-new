...
payload_text = os.getenv("MENU_TEXT", "")
if not payload_text:
    payload_text = "MacroDroid 알림 대기 중..."

# 식당 데이터 (주소 변환 없이 정확한 좌표를 직접 지정하여 오류 원천 차단)
scraped_data = []
...
# 4. 오정 (원하지 않으시면 이 블록 전체를 삭제하시면 됩니다!)
onjeongchan_html = """
...
.reset-map-btn {
    ...
    background: #fff; border: 3px solid #000; padding: 10px 16px;
    font-weight: bold; border-radius: 10px; cursor: pointer;
    box-shadow: 0px 4px 10px rgba(0,0,0,0.3); font-size: 15px;
}
</style>
<script>
let mapObj = null;
let initialCenter = [37.4790, 126.8820];
let initialZoom = 16;
...
for data in scraped_data:
    popup_html = f"""
    <div style="width:300px; text-align:center; padding:5px; cursor:pointer;" onclick="if(window.mapObj) {{ window.mapObj.closePopup(); }}">
    ...
    custom_icon = folium.DivIcon(
        html=f'<div style="background:#fff; border:2px solid #000; padding:5px 10px; border-radius:6px; font-weight:bold; box-shadow:0 2px 5px rgba(0,0,0,0.3); font-size:13px; white-space:nowrap;">{data["name"]}</div>'
    )
    
    folium.Marker(
        location=[data["lat"], data["lng"]],
        popup=folium.Popup(popup_html, max_width=340, close_onclick=False),
        tooltip=data["name"],
        icon=custom_icon
    ).add_to(menu_map)
